...
cursor = conn.cursor()
select_item = 'SELECT resource_id FROM value WHERE property_id = %s AND value = %s LIMIT 1'
with open('transcriptions.csv', newline='') as csv_file:
    reader = csv.DictReader(csv_file)
    for csv_row in reader:
        cursor.execute(select_item, (constants.PROPERTY_ID_IMAGE_ORIGINAL_PATH, csv_row['relative_path_to_img']))
